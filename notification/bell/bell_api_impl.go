...
func (g *gatewayApi) SendBell2(ctx context.Context, payload NotificationPayloads) error {
	start := time.Now()
	defer func() {
		log.Printf("sendNotif took %v", time.Since(start))
	}()
...
func (g *gatewayApi) SendBellBroadcast2(ctx context.Context, userIdentifiers []UserIdentifier, payload NotificationPayloadBroadcast) error {
	if len(userIdentifiers) == 0 {
		return errors.New("user identifiers array is empty")
	}
